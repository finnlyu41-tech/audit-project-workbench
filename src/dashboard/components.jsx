...
export function UserGuide() {
  ...
  const sections = [
    { id: "start", title: "快速开始", summary: "先建立公司主档，再为每个报告期间建立独立的年度项目。", topics: [
      { title: "查找公司的历年项目", steps: ["在公司主档按报告年度、项目类型或负责人组合搜索，也可查看未归档或已归档项目。", "历年待清事项会直接打开对应卡片；归档来源仍只能查看。"], result: "搜索和筛选不改变业务资料，项目进度与工作区使用同一计算规则。" },
    ...
    { id: "projects", title: "公司与年度项目", summary: "一个公司主档可以承载多个互相独立的报告期间和委聘。", topics: [
      { title: "快速打开与返回", steps: ["点击左侧放大镜，或按 Cmd/Ctrl + K，组合输入公司、年度、项目类型和负责人。", "使用上下方向键选择、Enter 打开、Esc 取消；历史资料需勾选包含归档记录。", "工作区前进和返回会保留本次会话的导航筛选、搜索、所选模块和工作区滚动位置，不会撤销业务修改。"], result: "切换项目不需要逐层展开公司，也不会自动保存未提交的快速编辑草稿。" },
      ...
      { title: "查找需要补日期的排期", steps: ["在排期页组合输入公司、报告年度、项目类型或负责人。", "选择日期待补，只显示缺少开始日或截止日的现有项目；清除筛选恢复当前状态范围。"], result: "搜索不改变日期、税务期限归属或已保存顺序，也不搜索备注。" },
      { title: "安排项目开始日和截止日", steps: ["打开年度项目设置，分别填写项目开始日和项目截止日；它们与报告期间是不同字段。",
        ...
        "按住项目名称所在的排期行拖动可调整显示顺序，键盘可按 Alt 加上下方向键移动；选择日历按钮或工期条可直接修改项目开始日和截止日，归档记录保持只读。",
        "滚轮上下查看项目，横向滚动条或触控板左右查看日期；选择“今天”定位到红色虚线。", "红色工期条代表已逾期，日期不完整的项目会显示提醒。"], result: "工作台会像年度计划表一样集中呈现项目起止时间，同时保留每个项目的详细流程。" },
    ...
    { id: "tax", title: "税务期限", summary: "把法定税务期限与内部项目排期分开管理。", topics: [
      { title: "保护税务期限编辑", steps: ["编辑税务期限后取消或关闭，会先确认是否放弃；清单筛选不算未保存更改。", "修改期限必须填写改期原因；保存后清除旧筛选并定位到该期限，原期限和历史继续保留。"], result: "草稿只在当前编辑器中，刷新或强制关闭不保证恢复。" },
      ...
      { title: "查找并筛选期限提醒", steps: ["打开铃铛后，可组合搜索公司、负责人、期限类型及年度，再按税务／项目和紧急程度筛选。", "清除提醒筛选会恢复已有提醒；点击某项可打开对应公司或项目。"], result: "筛选不会新增提醒或修改日期、提醒天数、完成状态；备注和参考编号不参与搜索。" },
    ...
    { id: "outstanding", title: "待清中心", summary: "待清事项独立于流程进度，可随审计过程持续变化。", topics: [
      ...
      { title: "在待清中心快速查找和定位", steps: ["按标题、来源公司、业务模块或来源项目负责人组合搜索；备注不会被搜索。", "保存表单后会清除遮住事项的面板筛选，并定位刚保存的卡片；取消保留原筛选。", "状态切换仍立即应用。事项离开当前列表后，键盘焦点转到相邻项或搜索框。"], result: "搜索只改变显示范围，不保存到业务数据，也不改变审计进度。" },
    ...
    { id: "reports", title: "管理层报告", summary: "从当前工作台资料实时生成内部进度及风险报告。", topics: [
      { title: "查看全部风险并打开来源", steps: ["每类风险先显示二十项；选择展开全部风险查看余下事项，筛选报告后会恢复收起。", "点击税务或待清风险直接定位对应期限或事项；展开和跳转不会修改业务资料。"], result: "打印包含当前报告范围内的全部风险，不受屏幕折叠影响。" },
    ...
    { id: "groups", title: "控股公司审计", summary: "当前公司层级与每个年度的合并范围分开保存。", topics: [
      { title: "查找本年度组成部分", steps: ["在组成部分面板按公司、角色或负责人搜索，可筛出待指定或期间不匹配的记录。", "指定项目会立即应用并清除面板筛选；历史范围只有明确同步后才会更新。"], result: "匹配提示只比较完整报告期间，不代表审计完成或已具备合并条件。" },
    ...
    { id: "templates", title: "范本与种类", summary: "范本用于快速建立新流程，修改范本不会改动既有项目。", topics: [
      ...
      { title: "检查范本包选择和导入决策", steps: ["导出时可以搜索范本名称、说明、标签、版本和种类；隐藏的已选范本仍会导出，底部会显示数量。", "全选全部范本和选择筛选结果是不同操作；清除搜索不会取消选择。", "修改导入方式、替换目标或种类后关闭，会先确认是否放弃。只有确认导入才会应用决策。"], result: "导入决策只在当前窗口保留；刷新或强制关闭不保证恢复。" },
      { title: "从指定范本新建项目", steps: ["在范本卡片点击使用此范本，选择兼容的现有公司或填写新公司。", "检查报告期间、项目类型和排期后确认建立；默认范本及既有项目不变。", "新公司与项目在最终确认时一并保存；取消不会留下空公司。"], result: "草稿只在本次流程内存中，刷新不能保证恢复。" },
    ...
    { id: "data", title: "保存、备份与数据", summary: "浏览器自动保存是默认方式，也可以关联一个持续同步的本地文件。", topics: [
      { title: "阅读操作提示", steps: ["普通操作提示显示在工作台顶部；打开弹窗时显示在标题下方，不会盖住保存或取消按钮。","提示显示 8–20 秒；悬停、键盘焦点停留或页面隐藏时暂停计时，也可以单独关闭提示。","只显示最近一条提示，不建立额外历史；新提示会重新计时。"], result: "关闭提示不会关闭编辑器或保存草稿；实际保存和同步状态仍以备份菜单为准。" },
      ...
      { title: "导出和恢复备份", steps: ["选择左侧窄工具栏的“备份”→“导出备份”，保存 JSON 文件。", "需要恢复时选择“恢复备份”并选取文件。",
        "恢复会替换当前资料；使用本地文件模式时也会更新关联文件，因此确认前应先导出当前备份。"], result: "备份包含公司主档、所有年度项目、控股关系、税务期限及其改期记录、范本、状态和自定义种类。" },
