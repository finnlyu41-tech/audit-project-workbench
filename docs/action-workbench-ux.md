...
Member cards have numbered accessible groups, equally sized 42px fields and 36px removal controls. Master fields share label, input and error tracks when an inline error is present (on browsers supporting CSS subgrid), so a name error does not push its neighbour out of alignment. Add/remove operations focus the corresponding new or surviving member; the last remaining row cannot be removed. New company creation clears navigation search and advanced filters, returns to company-list mode and focuses the new master. Cancelling does not change navigation filters or business data. Existing-company edits and the template-start company draft reuse the same required-name validation.
...
## Stable quick-update targets during draft feedback

Opt-in draft-recovery feedback follows the quick-update form rather than being inserted above it. A successful automatic draft write or a storage warning must not move Owner, dates, notes, Cancel or Save while the user is entering data or pressing a button. The recovery offer remains available when the editor is closed; the existing explicit restore, discard, save, conflict and archive rules are unchanged. This notice still means a temporary local draft, not a committed business record or a successfully synchronised file.

草稿保留提示移至快速更新表单之后，避免提示异步出现时把保存／取消按钮推离鼠标位置。恢复、丢弃和存储失败提示仍保留；不改变草稿默认关闭、七天期限、业务保存或文件同步语义。

The failed main CI run `34789140904`, attempt 1, commit `c983234b62b051c8935057e4b5f57b67f2ca6314`, retained its editor and draft after the Save click. The receipt, trace and screenshots show the 350 ms draft notice appearing around that gesture. Deterministic regressions in `efficiency-recovery.spec.js` release that existing callback between a real pointer-down and pointer-up, for new/restored drafts at 1440x900 and 800x560 in both browser engines. They compare layout coordinates separately from the existing 1px pressed-button transform, require the single gesture to save and clear the draft, compare the full workspace with only the intended notes/timestamp changes, then reload. No timeout, retry, persistence or business-data assertion is relaxed.
