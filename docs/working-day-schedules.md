...
## Use
Choose **Working days / 按工作天数** in the annual-engagement form, inline Quick update or timeline schedule editor. Enter a whole-number estimate (1–1000) and a requested start date. The end date is calculated immediately and is read-only in this mode. Manual dates remain the default for existing records without an estimate.

Three directions share the same calendar: start + duration calculates end, latest end + duration calculates start with an optional working-day buffer, and start + end counts available working days. The promised latest finish never moves automatically.
...
- Initial calendar: **Hong Kong**, explicitly selected Monday–Friday (default) or Monday–Saturday working week. Sundays and gazetted public holidays are always excluded; Saturdays are excluded in the five-day week. This is not the narrower statutory-holiday list, and not a statutory filing-deadline calculator.
...
- Whole days only. Personal leave, half-days, extra company holidays, other jurisdictions and inferred workload conflicts are not included. Selected sequential scheduling is explicit, not an automatic dependency engine.
...
Examples: 2026-09-30 + 3 working days ends 2026-10-05; 2026-04-02 + 2 ends 2026-04-08 (both 6 and 7 April are official replacement holidays); 2026-04-03 + 1 starts/ends 2026-04-08; 2026-12-31 + 2 ends 2027-01-04. The original request was issue #71. Unified delivery with backwards/counting and bulk scheduling is tracked in issue #73. See [the complete workflow guide](workflow-efficiency.md).
