...
- 年度项目可不启用任何业务模块而先作为空白委聘建立，也可并行启用账务处理、审计、税务及其他模块。负责人和日期统一由年度项目管理；业务模块只保存流程、完成条件和进度。业务模块、节点及完成条件均可拖动排序。点击模块才展开其节点，再点击可收起；点击节点才展开其里程碑条件，再点击同样可收起。 未展开的模块卡片直接显示已完成阶段数和按当前顺序的第一个未完成阶段；空模块显示尚未添加阶段，全部阶段完成时明确标示。缺少完成条件的阶段不算完成，待清数量仍独立显示；这些摘要不修改进度或用户自定义的下一步。
...
- An annual engagement may start blank with no workstream enabled. Bookkeeping, audit, tax and other optional parallel workstreams keep workflow, objective milestone criteria and progress, while owner and schedule stay at annual-engagement level. Workstreams, stages and criteria can all be reordered by drag or keyboard. Selecting a workstream reveals its stages; selecting a stage reveals its milestone criteria, and either level can be collapsed again. Collapsed cards also show the completed-stage count and the first unfinished stage in the saved order. Empty modules and fully completed workflows are labelled explicitly. A stage without criteria remains unfinished; outstanding counts stay separate. These summaries do not change progress or the engagement's pinned next action.
