...
Each `entity` is a company or holding company and owns its legal name, current parent relationship, fiscal-year default, tax deadlines and master lifecycle. Each `engagement` belongs to exactly one entity and owns the authoritative reporting-period dates, framework, owner, delivery schedule, workstreams, outstanding items and progress. An entity can exist without an engagement and can own any number of distinct reporting periods; archived engagements also participate in exact-period duplicate checks.
