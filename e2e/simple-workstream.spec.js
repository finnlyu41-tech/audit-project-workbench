...
test('switch mode, edit whole-module details, persist status and restore the original workflow', async ({ page }) => {
  ...
  await dialog.getByRole('combobox', { name: 'Workstream mode', exact: true }).selectOption('simple');
  ...
  expect(current.engagements[0].workstreams[1]).toEqual(before.engagements[0].workstreams[1]);
  expect(current.engagements[0].outstandingItems).toEqual(before.engagements[0].outstandingItems);
  dialog = await settings(page);
  await dialog.getByLabel('Workstream mode').selectOption('full');
  await expect(dialog.getByLabel('Workstream status')).toHaveCount(0);
  await dialog.getByRole('button', { name: 'Save workstream', exact: true }).click();
  ...
  expect((await savedModule(page)).nodes).toEqual(original.nodes);
  dialog = await settings(page);
  await dialog.getByLabel('Workstream mode').selectOption('simple');
